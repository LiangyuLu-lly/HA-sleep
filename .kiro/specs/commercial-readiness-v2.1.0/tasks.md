...
- [x] 2. Branding / Translations / Legal / Docs 资产
  ...
  - [x] 2.9 重写 README.md 顶部与商业化 sections继续检查当前剩余任务，全量完成
  ...
  - [x] 2.10 在 sleep_classifier/DOCS.md 增加 Limitations 与 disclaimer 链接
...
- [x] 3. Checkpoint - 基础守护层就绪
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. 新增 src/ 模块（onboarding_scanner / telemetry_reporter / upgrade_notifier / lovelace_template）
  ...
  - [x] 4.3 编写 onboarding_scanner 测试（Property 2）
  ...
  - [x] 4.4 在 src/ha_api_client.py 增加 Lovelace WebSocket 方法
  ...
  - [x] 4.5 实现 src/telemetry_reporter.py
  ...
  - [x] 4.6 编写 build_payload 不泄露 entity_id 测试（Property 3）
  ...
  - [x] 4.7 编写 telemetry 默认零外部出站测试（Property 4 — telemetry 部分）
  ...
  - [x] 4.8 编写 telemetry enable/disable 幂等测试（Property 6）
  ...
  - [x] 4.9 实现 src/upgrade_notifier.py
  ...
  - [x] 4.10 编写 is_newer 版本比较测试（Property 5）
  ...
  - [x] 4.11 编写 upgrade_notifier 默认零外部出站测试（Property 4 — upgrade 部分）
  ...
  - [x] 4.12 编写 upgrade_notifier 行为示例测试
  ...
  - [x] 4.13 实现 sleep_classifier/lovelace_template.py
  ...
  - [x] 4.14 静态测试 lovelace_template 引用完备性（P8.1）
  ...
  - [x] 4.15 编写 SleepStatePublisher / LearningPanelPublisher sensor 契约 snapshot 测试（PR2）
...
- [x] 5. Checkpoint - 新增 src/ 模块完成
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Web UI 集成与 config schema
  - [x] 6.1 在 sleep_classifier/web_ui.py 增加 onboarding wizard 路由
    - 增加：
  ...
  - [x] 6.2 编写 onboarding wizard 步骤渲染与状态恢复测试
  ...
  - [x] 6.3 编写 onboarding wizard 完成幂等与 dry_run 安全测试（Property 7 + Property 8）
  ...
  - [x] 6.4 在 sleep_classifier/web_ui.py 增加 Lovelace dashboard importer 路由
  ...
  - [x] 6.5 编写 dashboard importer 4 种组合 + 回滚测试
  ...
  - [x] 6.6 在 sleep_classifier/web_ui.py 增加 telemetry 开关与 upgrade banner 路由
    - 增加：
  ...
  - [x] 6.7 静态守护 web_ui.py 不直接 Path.write_text 写 /data（PR3）
  ...
  - [x] 6.8 更新 sleep_classifier/config.yaml schema
    - 在 `options:` 块追加：
      ```yaml
...
- [x] 7. 主入口接入与 CI 流水线
  - [x] 7.1 集成 telemetry / upgrade task 到 scripts/run_ha_smart_service.py
    - 在启动序列中注册：
      ```python
  ...
  - [x] 7.2 编写 run_ha_smart_service 启动 / 停机集成测试
  ...
  - [x] 7.4 扩展 .github/workflows/test.yml
  ...
  - [x] 7.5 新增 .github/workflows/addon-build.yml
  ...
  - [x] 7.6 新增 .github/workflows/release.yml
  ...
  - [x] 7.8 跑 prepare 同步 src/ scripts/ 到 sleep_classifier/rootfs/
